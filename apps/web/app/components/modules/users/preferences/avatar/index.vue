  ...
  <UPageCard variant="subtle" :ui="{ footer: 'flex flex-row justify-end w-full', body: 'w-full' }">
    <template #header>
      <LayoutPageCardHeader
        icon="i-tabler-photo"
        :title="$t('modules.users.preferences.avatar.title')"
        :description="$t('modules.users.preferences.avatar.description')"
      />
    </template>
    ...
    <template #body>
      <UForm :schema="schema" :state="state" class="space-y-4 w-64">
        <UFormField
          name="avatar"
          :label="$t('modules.users.preferences.avatar.form.fields.avatar.label')"
          :description="$t('modules.users.preferences.avatar.form.fields.avatar.description')"
        >
...
import type { UserProfileDTO } from '~/types/api/modules/user-profile';
...
const avatarPreviewUrl = computed<string | undefined>(() => {
  return croppedPreviewUrl.value ?? props.profile.avatarUrl;
});
const hasPersistedAvatar = computed<boolean>(() => !!props.profile?.avatarUrl);
...
const handleSubmit = async () => {
  try {
    ...
    const res = await useApi().put(`/users/${props.userId}/avatar`, formData);

    if (res.status === 200) {
      toasts.success(t('modules.users.preferences.avatar.form.actions.upload-success'));
      clearCroppedState();
    } else {
...
const handleDeleteAvatar = async () => {
  try {
    ...
    const res = await useApi().delete(`/users/${props.userId}/avatar`);
    if (res.status === 200 || res.status === 204) {
      clearCroppedState();
      // await authStore.getCurrentUser();
      await refresh();
      toasts.success(t('modules.users.preferences.avatar.form.actions.delete-success'));
    } else {
